...
from mixbag import chalk
from mixbag.chalk.utils import (
    BackgroundColor,
    Chalk,
    Color,
    FontFormat,
...
class TestChalkUtils(unittest.TestCase):
    ...
    def test_make_code_with_opts(self):
        ...
        actual = (
            ForegroundColor("green") + BackgroundColor("magenta") + chalk.bold
        )
    ...
    def test_format_txt_accepts_unicode(self):
        actual = chalk.white("abcd" + "つ", background="black")
        expected = u"\x1b[37;40mabcd\u3064\x1b[0m"
        self.assertEqual(actual, expected)

    def test_existence_of_needed_functions(self):
        """ref test name"""
        for color in COLORS:
            self.assertTrue(
                hasattr(chalk, color),
